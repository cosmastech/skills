...
- Application policy, authorization, or side-effect sequence: **Application Service Module**.
...
Design a meaningful service from its explicit interface first. In plain TypeScript, use a service interface and implementation class. Reserve service interfaces for helpers that own an application capability.
...
Application Services own policy and side-effect ordering while delegating substantial domain calculations and reusable mechanisms to their owning modules. Their operation bodies make sequence and decision points visible without accumulating unrelated implementations or introducing pass-through modules.
...
Use the repository's active complexity policy, preserving visible domain decisions and side-effect order. Extract a helper when it gives one calculation, translation, persistence operation, or policy a clearer owner. Retain exact domain inputs, expected-error unions, dependency requirements, and transaction/resource scopes.
...
**Complete when:** every extraction passes the deletion test, every moved side effect retains its ordering/lifetime/failure semantics, and each removed operation has consumer and invariant evidence. Improve the design rather than weakening lint or partitioning one operation merely to fit a metric.
