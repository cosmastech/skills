...
- framework-supported dependency injection;
...
Use `fast-check` in TypeScript projects.
...
When inference is public behavior, add compile-time tests using ordinary call sites without rescue annotations or casts. For parser/service API changes, assert the complete function contract: input, arity/options, success, and expected errors. Include rejected calls as well as inferred results; parameter-only assertions can miss an error type widened by extraction.
