...
description: Correct-by-construction TypeScript standards for types, errors, parsing, services, persistence, testing, and maintainability. Activate whenever creating, modifying, reviewing, or planning TypeScript code.
---
...
- [`references/modules-services-and-adapters.md`](references/modules-services-and-adapters.md) — when assigning ownership, extracting helpers, reducing complexity, or changing dependencies, module/service design, or side-effect order.
...
**Complete when:** every changed input, output, failure, dependency, side effect, state transition, external representation, and test surface maps to an owning module and an applicable reference.
...
**Complete when:** caller-facing inputs, outputs, expected errors, and service interfaces are explicit; every changed dependency and side effect has one owner; each new abstraction has deletion-test evidence for the final report; and framework/provider types remain private to their owner.
